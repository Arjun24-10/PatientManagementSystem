...
import {
   Calendar, Clock, MapPin, Plus, AlertCircle, User, Building2, FileText, X, Check, ChevronUp, ChevronRight, ChevronLeft, Bell, Download, List, LayoutGrid,
   RefreshCw,
} from 'lucide-react';
...
const PatientAppointments = () => {
   ...
   const renderAppointmentCard = (appt) => {
      ...
      return (
         <Card
            key={appt.id}
            className="p-5 flex flex-col md:flex-row justify-between items-start md:items-center hover:shadow-lg transition-all duration-200 border border-gray-100"
         >
      ...
      <div className="space-y-6">
                        ...
                        upcomingReminders.map(appt => (
                           ...
                           >
                              <div className="flex justify-between items-start mb-2">
                                 <p className="font-semibold text-sm text-gray-800">{appt.type}</p>
                                 <Badge type="green" className="text-xs">
                                    {getCountdown(appt)}
                                 </Badge>
                              </div>
                              <p className="text-xs text-gray-600 mb-1">{appt.doctorName}</p>
         ...
         >
            <form onSubmit={handleRequestSubmit} className="space-y-6">
               ...
               <div className="flex items-center justify-between mb-6">
                  {[1, 2, 3].map(step => (
                     <div key={step} className="flex items-center flex-1">
                        ...
                        {step < 3 && (
                           <div
                              className={`flex-1 h-1 mx-2 ${step < requestStep ? 'bg-blue-600' : 'bg-gray-200'
                                 }`}
                           ></div>
                        )}
                     </div>
               ...
               {requestStep === 1 && (
                  <div className="space-y-4">
                     <h3 className="font-bold text-lg text-gray-800">Select Department & Doctor</h3>
               ...
               {requestStep === 2 && (
                  <div className="space-y-4">
                     <h3 className="font-bold text-lg text-gray-800">Choose Date & Time</h3>
