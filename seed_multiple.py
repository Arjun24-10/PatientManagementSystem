...
def register_user(email, password, role, name, dob):
    data = {"email": email, "password": password, "role": role, "full_name": name, "date_of_birth": dob, "address": "123 Random Address"}
...
# Register Doctors
doctors = [
    {"email": "dr.smith@securehealth.com", "name": "Dr. Alice Smith", "dob": "1975-06-21"},
    {"email": "dr.jones@securehealth.com", "name": "Dr. Bob Jones", "dob": "1980-03-12"},
    {"email": "dr.davis@securehealth.com", "name": "Dr. Carol Davis", "dob": "1985-11-05"},
]
for d in doctors:
    register_user(d["email"], "Password123xyz!", "DOCTOR", d["name"], d["dob"])

# Register Nurses
nurses = [
    {"email": "nurse.joy@securehealth.com", "name": "Nurse Joy", "dob": "1990-01-01"},
    {"email": "nurse.jack@securehealth.com", "name": "Nurse Jack", "dob": "1992-04-18"},
]
for n in nurses:
    register_user(n["email"], "Password123xyz!", "NURSE", n["name"], n["dob"])

# Register Admins
admins = [{"email": "admin.max@securehealth.com", "name": "Admin Max", "dob": "1975-01-01"}]
for a in admins:
    register_user(a["email"], "Password123xyz!", "ADMIN", a["name"], a["dob"])

# Register Patients
patients = []
for i in range(1, 11):
    patients.append({"email": f"patient{i}@securehealth.com", "name": f"Test Patient {i}", "dob": f"19{70+i}-05-1{i}"})
for p in patients:
    register_user(p["email"], "Password123xyz!", "PATIENT", p["name"], p["dob"])
...
with open("seed_multiple.sql", "w") as f:
    f.write(sql_commands)

print("Executing SQL in Docker...")
subprocess.run('docker exec -i secure_health_db psql -U postgres -d healthcare_auth_db < seed_multiple.sql', shell=True)
print("Seeding of multiple users complete!")
